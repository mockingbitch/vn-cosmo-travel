...
@foreach ($codes as $loc)
    @php
        ...
        $block = is_array($oldTranslations[$loc] ?? null)
            ? $oldTranslations[$loc]
            : [];
        $blockTitle = isset($block['title']) ? (string) $block['title'] : $about->blockForLocale((string) $loc)['title'];
        $blockContent = isset($block['content']) ? (string) $block['content'] : $about->blockForLocale((string) $loc)['content'];
    @endphp
