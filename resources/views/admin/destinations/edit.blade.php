...
@section('content')
    <h1 class="text-2xl font-semibold tracking-tight">{{ __('Edit destination') }}</h1>

    <form method="POST" action="{{ route('admin.destinations.update', $destination) }}" class="mt-6 max-w-2xl space-y-4">
        @csrf
        @method('PUT')
